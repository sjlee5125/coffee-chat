...
def generate_wrapup_report(host_text: str, guest_text: str, host_name: str = "멘토", guest_name: str = "멘티") -> str:
    ...
    **3. 🔄 [실전 활용] 실무 언어 번역기 (Academic vs Practical)**
    ...
    - [중요 지시사항] 반드시 아래의 3열(Column) 마크다운 표 형식을 엄격하게 유지하며, 헤더 아래의 구분선(`|---|---|---|`)을 절대 생략하지 마세요.
    ...
    **5. 📋 [액션 플랜] 나만의 마이크로 체크리스트**
    ...
    - [중요 지시사항] 반드시 아래의 4열(Column) 마크다운 표 형식을 엄격하게 유지하며, 헤더 아래의 구분선(`|:---:|:---|:---:|:---|`)을 절대 생략하지 마세요.
...
def generate_summary(text: str) -> str:
    """대화 내용을 짧게 요약해주는 함수"""
    if not text or text == "이 대화는 텍스트 기록이 없습니다.":
        return "대화 내용이 부족하여 요약할 수 없습니다."
        
    system_prompt = "다음 대화 내용을 바탕으로 어떤 고민을 나누었고 어떤 결론이 났는지 3~4줄로 핵심만 명확하게 요약해 주세요."
    ...
    try:
        ...
        print(f"Azure OpenAI 요약 에러: {e}")
        return "정보 부족 (요약 생성 중 에러가 발생했습니다.)"